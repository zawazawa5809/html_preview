...
test('リサイズハンドル: ドラッグで要素の幅を変更できる（PointerEvent）', async ({ page }) => {
  const preview = page.frameLocator('#preview-container');
  const target = preview.locator('.highlight').first();
  await target.click();

  const before = (await target.boundingBox()).width;
  const handle = preview.locator('.designer-resize-handle[data-dir="e"]');
...
test('書式ツールバー: テキスト選択で表示され、太字を適用できる', async ({ page }) => {
  const preview = page.frameLocator('#preview-container');
  const p = preview.locator('p').first();
  await p.click({ clickCount: 3 }); // トリプルクリックで段落テキストを選択

  const boldBtn = preview.locator('[data-cmd="bold"]');
