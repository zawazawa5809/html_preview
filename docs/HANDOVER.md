...
- テスト: ユニット13ファイル / 128テスト（`npm test`）+ E2E 15テスト（`npm run test:e2e`）。全GREEN
- 品質ゲート: ESLint（`npm run lint`）/ Prettier（`npm run format:check`）/ カバレッジ閾値（`npm run test:coverage`）がCIで強制される
- 改善バックログ（下記）のP1全件・P2全件・P3の2件は実施済み
...
4. ~~**モバイル/タッチでのDesign Mode操作**~~ ✅（2026-06-12・一部残課題あり）
   リサイズハンドル・gutterドラッグを PointerEvent + `touch-action:none` に統一（タッチ対応）。書式ツールバーは `selectionchange` ベースに変更しタッチ/キーボード選択にも反応。**残課題**: 要素並べ替えD&DはHTML5 DnD依存のためタッチ未対応（対応にはpointerベースの並べ替え再実装が必要）

### 残課題（新規追加分）

- 「信頼しないHTMLを開くモード」（sandbox化＋Design Mode無効）の追加検討（P2-4の将来対応）
- 要素並べ替えD&Dのタッチ対応（P3-4の残り）
